...
class PostAdapter(private var listPost: ArrayList<Post>, private var listener: (Post) -> Unit)
    : RecyclerView.Adapter<PostAdapter.RecyclerViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PostAdapter.RecyclerViewHolder {
        val binding = ItemCommunityPostBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return RecyclerViewHolder(binding)
    }

    override fun onBindViewHolder(holder: PostAdapter.RecyclerViewHolder, position: Int) {
