...
import com.dhilasdrh.zerowaste.ui.shop.farmersmarket.FarmersMarketFragment
...
class ShopPagerAdapter(private val mContext: ShopFragment, fm: FragmentManager): FragmentPagerAdapter(fm, BEHAVIOR_RESUME_ONLY_CURRENT_FRAGMENT){
    @StringRes
    private val tabTitle = intArrayOf(R.string.rescue_food, R.string.farmer_market, R.string.thrift_shop, R.string.upcycled)

    override fun getItem(position: Int): Fragment {
        var fragment: Fragment? = null
        when(position){
            0 -> fragment = RescueFoodFragment()
            1 -> fragment = FarmersMarketFragment()
